...
* 首次渲染后：元素还可能被不断的重新布局、重新绘制。
    ...
    * CSS样式修改：
        ...
        * 其他属性：(transform | opacity)这种特别的属性可以在不同的层中单独绘制，不会触发Layout-Paint，而是直接进行Composite合并图层。而要实现对这个元素创建一个独立的渲染层，必须通过配置will-change来提升该元素。然而太多的呃独立渲染层，虽然缩减了Paint的时间，但是增加了Composite Layers的时间，因此需要配合Timeline的运行结果来仔细权衡。
